...
class FiberPhotometryData:
    ...
    def __init__(
        ...
        """Initialize Fiber photometric data object"""
        ...
        missing_columns = [
            ...
            if col not in fluorescence_df.columns
        ]
        if missing_columns:
            raise ValueError(
        ...
        self._floor_avg = self.fluorescence_df[self.background_channel].mean()
...
class AbstractMetricEvaluator(ABC):
    ...
    def stage(self) -> Stage:
        """Return the QC stage for this metric. Override in subclasses if needed."""  # noqa: E501
        return Stage.RAW
    ...
    def evaluate(self, data: FiberPhotometryData) -> QCMetric:
        ...
        qc_status = QCStatus(
            ...
            status=status,
            timestamp=datetime.now(timezone.utc),  # noqa: E501
        )
...
class CMOSFloorDarkMetric(AbstractMetricEvaluator):
    ...
    def __init__(
        ...
        """Initialize CMOSFloorDarkMetric object"""
        super().__init__(evaluator_name)
        self.limit = limit
...
class MetricRegistry:
    ...
    def get_metric(cls, class_name: str):
        ...
        if class_name not in cls._metrics:
            raise ValueError(
                f"Metric class '{class_name}' not found. Available: {list(cls._metrics.keys())}"  # noqa: E501
            )
...
class DataIntegrityMetric(AbstractMetricEvaluator):
    ...
    def description(self) -> str:
        """Metric name"""
        return "Verify that fiber channel and background data have matching lengths"  # noqa: E501

    def _evaluate_condition(self, data: FiberPhotometryData) -> bool:
        """Evaluate condition"""
        return (
            len(data.fiber_data)
            == len(data.background_data)
            == len(data.time_data)
        )
...
class DataDurationMetric(AbstractMetricEvaluator):
    ...
    def description(self) -> str:
        """Metric description"""
        return "Verify that recording duration exceeds 15 minutes (>18000 samples)"  # noqa: E501
...
class CMOSFloorDarkFiberMetric(CMOSFloorDarkMetric):
    ...
    def __init__(
        ...
        """Initialize CMODFloorDarkFiberMetric object"""
...
class NoSuddenChangeMetric(AbstractMetricEvaluator):
    ...
    def __init__(
        ...
        """Initialize NoSuddenChangeMetric object"""
        super().__init__(evaluator_name)
        self.limit = limit
    ...
    def description(self) -> str:
        """Metric description"""
        return f"Verify that maximum signal change is below {self.limit} in fiber channel"  # noqa: E501
    ...
    def _evaluate_condition(self, data: FiberPhotometryData) -> bool:
        ...
        try:
            ...
            trimmed_data = (
                fiber_data[10:-2] if len(fiber_data) > 12 else fiber_data
            )
...
class SingleRecordingMetric(AbstractMetricEvaluator):
    ...
    def description(self) -> str:
        """Metric description"""
        return (
            "Verify that the data represents a single valid recording session"
        )
...
class FiberPhotometryQCEvaluator:
    """Main evaluator class that manages and runs all QC metrics
    ...
    def __init__(
        ...
        metric_kwargs: dict = None,  # noqa: E501
    ):
        """Initialize FiberPhotometryQCEvaluator"""
        self.evaluator_name = evaluator_name
        self.raw_metric_names = raw_metrics or self.DEFAULT_RAW_METRICS.copy()
        self.processed_metric_names = (
            processed_metrics or self.DEFAULT_PROCESSED_METRICS.copy()
        )
        self.metric_kwargs = metric_kwargs or {}

    def _create_metrics_from_strings(
        self, metric_names: List[str]
    ) -> List[AbstractMetricEvaluator]:
        ...
        for name in metric_names:
            ...
            kwargs["evaluator_name"] = kwargs.get(
    ...
    def get_processed_metrics(self) -> Iterator[AbstractMetricEvaluator]:
        """Get iterator for processed data metrics created from string names."""  # noqa: E501
        return iter(
            self._create_metrics_from_strings(self.processed_metric_names)
        )
    ...
    def get_current_metrics(self):
        ...
        return {
            ...
            "processed": self.processed_metric_names.copy(),  # noqa: E501
